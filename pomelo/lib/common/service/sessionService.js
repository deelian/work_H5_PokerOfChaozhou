...
SessionService.prototype.sendMessage = function(sid, msg) {
  ...
  if(!session) {
    logger.debug('Fail to send message for non-existing session, sid: %j msg: %j', sid, msg);
    return false;
  }
...
Session.prototype.closed = function(reason) {
  logger.debug('session on [%s] is closed with session id: %s reason: %j', this.frontendId, this.id, reason);
