 ...
 * @param {Array} uid the receiver info list
 * @param {Array} sid the receiver frontendId
 * @param {Object} opts user-defined push options, optional
 * @param {Function} cb cb(err)
 * @memberOf ChannelService
 */
ChannelService.prototype.pushMessage = function(route, msg, uid, sid, opts, cb) {
    var groups = {};
...
Channel.prototype.broadcast = Channel.prototype.pushMessage;
Channel.prototype.send = function(uid, route, msg, opts, cb) {
    if (this.state !== ST_INITED) {
        utils.invokeCallback(new Error('channel is not running now'));
        return;
    }
    ...
    var groups = {};
    var record = this.getMember(uid);
    if (record) {
        groups[record.sid] = [uid];
    }

    sendMessageByGroup(this.__channelService__, route, msg, groups, opts, cb);
};
...
var sendMessageByGroup = function(channelService, route, msg, groups, opts, cb) {
    ...
    logger.debug('channelService sendMessageByGroup route: %j, msg: %j, groups: %j, opts: %j', route, msg, groups, opts);
    if(count === 0) {
        // group is empty
        utils.invokeCallback(cb);
        return;
